      ...
      for (let c in ProgramList) {
        for (let d in SheetData) {
          if (ProgramList[c] == SheetData[d][11]) {
            ...
            Row = Number(d) + 3;
            await RPA.Logger.info(`①${Column} 列 :${Row} 行目`);
            ...
            for (let e in Title) {
              if (Title[e][0] != undefined) {
                ...
                await RPA.Logger.info(SheetData[Row2 - 1][Column2]);
...
async function Work() {
  try {
    ...
    if (
      ...
      await RPA.Logger.info(`番組名が【${CurrentProgramName}】です`);
      await Spot();
...
async function SetData(ShityouKaisuu) {
  await RPA.Logger.info(`シートID：${LastMonthSheetID}`);
  await RPA.Logger.info(`シート名：${CurrentSSName}`);
