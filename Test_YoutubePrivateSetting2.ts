...
async function Start() {
  if (ErrorText.length == 0) {
    ...
    await RPA.Logger.info(Yesterday);
...
async function VideoDate(Yesterday) {
  try {
    ...
    // 本番用
    // const Videodate = await RPA.WebBrowser.findElementById('text-item-9');
    ...
    //   RPA.WebBrowser.Until.elementLocated({ xpath: '//*[@id="input-1"]/input' }),
    //   5000
    // );
